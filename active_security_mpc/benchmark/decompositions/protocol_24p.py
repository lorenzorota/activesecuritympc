from .additive_24p import get_additive_shares
from .additive_24p import reconstruct_secret
...
def protocol_1(
    ...
    Args:
        mixed_shares (Private[Array[field, 24]]): The received secret shares (including the initial secret).
        _one (Public[field]): The multiplicative identity.
...
def protocol_2(
    final_shares: Private[Array[field, 24]],
    _one: Public[field],
) -> field:
    ...
    Args:
        final_shares (Private[Array[field, 24]]): The received combinations (including the locally produced one).
        _one (Public[field]): The multiplicative identity.
