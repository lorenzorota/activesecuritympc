from .additive_33p import get_additive_shares
from .additive_33p import reconstruct_secret
...
def protocol_2(
    ...
    Args:
        mixed_shares (Private[Array[field, 33]]): The received secret shares (including the initial secret).
        _one (Public[field]): The multiplicative identity.
...
def protocol_3(
    final_shares: Private[Array[field, 33]],
    _one: Public[field],
) -> field:
    ...
    Args:
        final_shares (Private[Array[field, 33]]): The received combinations (including the locally produced one).
        _one (Public[field]): The multiplicative identity.
