from .additive_38p import get_additive_shares
from .additive_38p import reconstruct_secret
...
def protocol_2(
    ...
    Args:
        mixed_shares (Private[Array[field, 38]]): The received secret shares (including the initial secret).
        _one (Public[field]): The multiplicative identity.
...
def protocol_3(
    final_shares: Private[Array[field, 38]],
    _one: Public[field],
) -> field:
    ...
    Args:
        final_shares (Private[Array[field, 38]]): The received combinations (including the locally produced one).
        _one (Public[field]): The multiplicative identity.
