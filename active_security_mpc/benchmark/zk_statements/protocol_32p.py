...
from ..decompositions.protocol_32p import protocol_0, protocol_1, protocol_2
...
def auth_protocol_0(
    ...
    blinding_factors: Private[Array[field, 64]],
    party_index: Public[int],
    _one: Public[field],
) -> Array[Array[int, 8], 32]:
    """Private protocol authentication for protocol 0"""
    outputs_comm: Array[Array[int, 8], 32] = [[0 for _ in range(8)] for _ in range(N)]
    outputs: Array[field, 32] = protocol_0(secret, randomness, party_index, _one)
...
def auth_protocol_1(
    private_input: Private[Array[field, 32]],
    blinding_factors: Private[Array[field, 64]],
    public_input_comm: Public[Array[Array[int, 8], 32]],
    _one: Public[field],
) -> field:
...
def auth_protocol_2(
    public_input: Public[Array[field, 32]],
    _one: Public[field],
) -> field:
