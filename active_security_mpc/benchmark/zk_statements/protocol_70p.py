...
from ..decompositions.protocol_70p import protocol_0, protocol_1, protocol_2

N: int = 70 # Hardcoded number of parties

# ZK-statements for protocol authentication

def auth_protocol_0(
    secret: Private[field],
    randomness: Private[Array[field, 69]],
    blinding_factors: Private[Array[field, 140]],
    party_index: Public[int],
    _one: Public[field],
) -> Array[Array[int, 8], 70]:
    """Private protocol authentication for protocol 0"""
    outputs_comm: Array[Array[int, 8], 70] = [[0 for _ in range(8)] for _ in range(N)]
    outputs: Array[field, 70] = protocol_0(secret, randomness, party_index, _one)
...
def auth_protocol_1(
    private_input: Private[Array[field, 70]],
    blinding_factors: Private[Array[field, 140]],
    public_input_comm: Public[Array[Array[int, 8], 70]],
    _one: Public[field],
) -> field:
...
def auth_protocol_2(
    public_input: Public[Array[field, 70]],
    _one: Public[field],
) -> field:
