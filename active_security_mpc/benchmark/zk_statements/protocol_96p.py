...
from ..decompositions.protocol_96p import protocol_0, protocol_1, protocol_2

N: int = 96 # Hardcoded number of parties

# ZK-statements for protocol authentication

def auth_protocol_0(
    secret: Private[field],
    randomness: Private[Array[field, 95]],
    blinding_factors: Private[Array[field, 192]],
    party_index: Public[int],
    _one: Public[field],
) -> Array[Array[int, 8], 96]:
    """Private protocol authentication for protocol 0"""
    outputs_comm: Array[Array[int, 8], 96] = [[0 for _ in range(8)] for _ in range(N)]
    outputs: Array[field, 96] = protocol_0(secret, randomness, party_index, _one)
...
def auth_protocol_1(
    private_input: Private[Array[field, 96]],
    blinding_factors: Private[Array[field, 192]],
    public_input_comm: Public[Array[Array[int, 8], 96]],
    _one: Public[field],
) -> field:
...
def auth_protocol_2(
    public_input: Public[Array[field, 96]],
    _one: Public[field],
) -> field:
