...
from ..decompositions.protocol_40p import protocol_1, protocol_2, protocol_3

N: int = 40 # Hardcoded number of parties

# Active security components
def engage_protocol_1(
    secret: Private[field],
    randomness: Private[Array[field, 39]],
    blinding_factors: Private[Array[field, 80]],
    party_index: Public[int],
    _one: Public[field],
) -> Array[Array[int, 8], 40]:
    """Engagement of secret in protocol 1."""
    outputs_comm: Array[Array[int, 8], 40] = [[0 for _ in range(8)] for _ in range(N)]
    outputs: Array[field, 40] = protocol_1(secret, randomness, party_index, _one)
...
def auth_protocol_2(
    private_input: Private[Array[field, 40]],
    blinding_factors: Private[Array[field, 80]],
    public_input_comm: Public[Array[Array[int, 8], 40]],
    _one: Public[field],
) -> field:
...
def auth_protocol_3(
    public_input: Public[Array[field, 40]],
    _one: Public[field],
) -> field:
